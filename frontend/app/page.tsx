...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const res = await fetch("http://localhost:8000/executive-dashboard?force_refresh=true");
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-10 bg-zinc-950">
        <div className="text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-zinc-100 font-sans">Compiling Market Intelligence...</h2>
          <p className="text-zinc-500 text-sm">Synchronizing scoring engines, forecasts, and YC partner blueprints.</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 p-10 bg-zinc-950 max-w-7xl mx-auto w-full flex flex-col justify-center">
        <div className="border border-zinc-800 bg-zinc-900/40 backdrop-blur-md rounded-xl p-8 max-w-xl mx-auto text-center space-y-4">
          ...
          <h2 className="text-2xl font-bold text-zinc-100">Analytics Engine Offline</h2>
          <p className="text-zinc-400 text-sm leading-relaxed">
            The Startup Radar database is initializing or offline. Ensure the backend server is running, or trigger a manual synchronization in Settings.
          </p>
          <div className="pt-4 flex items-center justify-center space-x-4">
            <Link
              href="/settings"
              className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-lg bg-indigo-600 hover:bg-indigo-500 font-semibold text-sm transition-all"
            >
              <span>Settings</span>
  ...
  const totalStarsCount = data.fastest_growing_categories.reduce((acc, curr) => acc + curr.star_count, 0);
  const totalNewsVolume = data.fastest_growing_categories.reduce((acc, curr) => acc + curr.news_volume, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card p-6 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="text-xs text-zinc-400 mt-4 font-semibold">
            Tracked across <span className="text-indigo-400 font-bold">{data.fastest_growing_categories.length} categories</span>
          </div>
        </div>
        ...
        <div className="glass-card p-6 flex flex-col justify-between relative overflow-hidden group">
          ...
          <div className="text-xs text-zinc-400 mt-4 flex items-center space-x-1 font-semibold">
            ...
            <span className="text-emerald-400 font-extrabold">+{topNiche ? topNiche.growth_rate : 0}%</span>
          </div>
        </div>
      </div>
